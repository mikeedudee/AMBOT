...
class TelemetryDashboard(QMainWindow):
    def __init__(self, theme):
        ...
        # Set up status update callback to refresh display when status changes
        status_store.set_status_update_callback(self._update_status_display)
        
        # Set up timer to periodically refresh status display
        ...
        # Initialize WiFi receiver (optional, can be enabled via settings)
        self.wifi_receiver = None
    
    def _update_status_display(self):
        """Update the status display with current status values."""
        try:
            sensor_states, servo_states = status_store.get_all_status()
            # Only update if we have valid states
            if sensor_states and servo_states:
                self.system_status.set_status(sensor_states, servo_states)
