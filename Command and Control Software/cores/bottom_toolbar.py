...
from cores.connection_port import ConnectionPort
from cores.status_store import status_store
from cores.keyboard_control import start_keyboard_control_thread
...
class BottomToolbar(QWidget):
    def __init__(self, parent=None):
        ...
        self.connection_port        = None  # ConnectionPort instance
        self._keyboard_thread       = None  # Keyboard control thread
    ...
    def on_manual_control_selected(self, idx):
        ...
        # Start keyboard control if keyboard control is selected and connected
        if idx == 0 and self.manual_mode_active:  # idx 0 = KEYBOARD CONTROL
            if self.connection_port and self.connection_port.is_connected:
                start_keyboard_control_thread(self)
    ...
    def on_connect_disconnect(self):
        ...
        if not self.connected:
            ...
            if not port_name:
                ...
                return
            try:
                # Create ConnectionPort instance for APC220 USB port
                self.connection_port = ConnectionPort(
                ...
                # Connect using ConnectionPort
                if self.connection_port.connect(port_name, baudrate):
                    # Also keep serial_port for backward compatibility
                    self.serial_port = self.connection_port.serial_port
                    self.connected = True
                    self.connect_btn.setText("CONNECTED")
                    self.connect_btn.setProperty("connection", "connected")
                    
                    # Start keyboard control if manual mode and keyboard control are selected
                    if self.manual_mode_active and self.selected_manual_idx == 0:
                        start_keyboard_control_thread(self)
                else:
            ...
            # Disconnect ConnectionPort if it exists
            ...
            if hasattr(self, '_keyboard_manager') and self._keyboard_manager:
                try:
                    self._keyboard_manager.stop()
                except:
                    pass
